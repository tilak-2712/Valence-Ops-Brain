...
So the only honest differentiation available is **method transparency**: showing exactly how the work gets done, on their own clinic, before any money changes hands, and being explicit about what isn't known yet. This is already the philosophy of `01 Playbooks/diagnostic_doc_playbook.md` §2 — *"here's what we could verify, here's what we couldn't, here's what we'd test if we had access."* This document extends that same posture across five stages instead of one page.
