...
**The Go/No-Go gate in `clinic-audit-checklist.md` cannot be satisfied by this skill alone** —
it requires the mystery shop. Every dossier ships as *provisionally complete*. Say so.
...
Run the low-cost hard-kill checks (Phase 6 §1.1) *before* deep enrichment. Two are answerable from a
single GBP pull:
...
| 9 | Passes all six speed-to-lead tests — *only determinable post-mystery-shop* |
...
Many §3 rows and **all six** speed-to-lead tests depend on response-speed data that only the
...
> "Follow-up and nurture engine" (§3 row 2) or the §2 six-test SOP instead.
