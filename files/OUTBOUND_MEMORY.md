...
| 2026-08-04 | **`Audit_call_docs/02_data_intake_requirements.md` re-staged to Stage-4 kickoff**, delivered in a live 45-min setup session rather than as a document. At the findings stage the ask is exactly one item, producible in ten minutes | Raw CRM export + Meta read-only + WhatsApp history + billing counts + staff list, requested from a clinic that hasn't agreed to anything, asks strangers to be handed patient data. Good document, wrong point in the sequence |
