...
- **Deal state lives in `04 Clients/<client>/<Client> Current State.md`, not in this file.** *(2026-09-15, confirmed.)* A session read §5 below, told Tilak the only warm threads were Sapphire and Aesthetica Veda with zero calls held, and missed three live deals (Aurilueur, RUA, SkinFit). He corrected it. Before any claim about pipeline, meetings held or client count, list `04 Clients/` and read each Current State note. §5 is history; the Current State notes and Notion win.
- **`/wrap` stays manual until 2026-09-15, then switches to automatic.** *(2026-09-08, confirmed.)* One week of visible changesets first so the routing can be checked against his judgement, then install the session-end hook without asking and tell him it was done. No session can act on a date by itself, so this entry is the trigger: the first session on or after 2026-09-15 makes the change. If `/wrap` has not run cleanly a few times by then, say so and hold rather than automate something neither of us has watched work.
  - *Correction 2026-09-15 (`confirmed`, git `0be0f1d` + `.claude/settings.json`):* done — the Stop hook runs `wrap-check.py` and the wrap skill is propose-only. Nothing left to trigger.
...
1. **In-person drop-ins convert well with clinic owners.** Zero meetings held, zero calls held. Actively being pursued. The ladder Tilak runs: drop-in → call if that's too much commitment → on-site once the audit is agreed.
   - *Correction 2026-09-15 (`confirmed` for meetings, `hypothesis` for conversion):* meetings now held — SkinFit in person with Drish and Gino, Aurilueur offline meeting plus a 69-min walkthrough call on 10 Sep, RUA offline with Dr. Jatin on 15 Sep. **None signed yet**, so "converts" is still unproven.
...
5. **The free audit + strategy doc earns the paid implementation.** Free has been offered ~25 times and accepted zero times, though never yet in this exact shape.
   - *Correction 2026-09-15:* overtaken. The free pilot is retired (`OUTBOUND_MEMORY.md` §5, 2026-09-05), SkinFit refused the audit and asked for costs, and Aurilueur is being closed on a paid 30-day pilot. `confirmed`
...
*Correction 2026-09-15 (`confirmed`, from `04 Clients/`):* no longer the only threads. **Three live deals, none signed:** Aurilueur (revised pilot proposal sent 13 Sep, closing call pending), RUA (first meeting 15 Sep, re-costed proposal owed in 3–4 days), SkinFit (final proposal sent, response unknown as of 8 Sep). Current state of each is in its `Current State.md`. Sapphire and Aesthetica Veda status not re-verified here — check Notion.
